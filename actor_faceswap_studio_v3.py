...
class MultiConfigTester:
    """Gestionnaire pour tester plusieurs configurations et comparer les résultats"""
    ...
    def run_multi_tests(
        ...
        test_pixel_boosts: List[str],
        test_enhancers: List[str],
        test_blends: List[int],
        ...
        for model in test_models:
            for pixel in test_pixel_boosts:
                for enhancer in test_enhancers:
                    for blend in test_blends:
                        for distance in test_distances:
                            config_name = f"{model[:10]}_{pixel.split('x')[0]}p_{enhancer[:4]}_b{blend}_d{str(distance).replace('.', '')}"
                            combinations.append({
                                'name': config_name,
                                'model': model,
                                'pixel': pixel,
        ...
        for i, config in enumerate(combinations):
            progress((i / total), desc=f"Test {i+1}/{total}: {config['name'][:30]}")

            output_path = str(self.test_session_dir / f"{config['name']}.mp4")

            # Construire commande
            cmd = self.processor.build_command(
                source_image_path, target_video_path, output_path,
                config['model'], config['pixel'],
                face_enhancer_enabled, config['enhancer'], str(config['blend']),
                face_detector_size, str(config['distance']),
                lip_sync_enabled, lip_sync_model,
                execution_provider, output_quality
            )

            print(f"\n{'='*60}\nTest {i+1}/{total}: {config['name']}\n{'='*60}")
            ...
            try:
                ...
                if result.returncode == 0 and os.path.exists(output_path):
                    ...
                    video_paths.append((output_path, config['name']))
                    print(f"✓ Succès: {elapsed:.1f}s, {size_mb:.1f}MB")
                else:
                    self.results.append({
                        'name': config['name'],
                        'status': '❌',
                ...
                self.results.append({'name': config['name'], 'status': '❌', 'error': str(e)})
                print(f"✗ Erreur: {e}")
        ...
        results_file = self.test_session_dir / "results.json"
        with open(results_file, 'w') as f:
            json.dump(self.results, f, indent=2, default=str)

        return summary, video_paths, str(self.test_session_dir)

    def _generate_summary(self):
        """Génère un résumé textuel"""
        successful = [r for r in self.results if r.get('status') == '✅']
        failed = [r for r in self.results if r.get('status') == '❌']

        summary = f"""
🎯 TESTS MULTI-CONFIGURATIONS TERMINÉS
{'='*60}
...
"""
        for i, r in enumerate(self.results, 1):
            if r.get('status') == '✅':
                ...
                summary += f"\n{i}. ❌ {r['name']}\n   Erreur: {r.get('error', 'Unknown')[:80]}\n"

        return summary


# Instance globale du multi-tester
multi_tester = MultiConfigTester(processor)
...
def create_gradio_interface():
    ...
    with gr.Blocks(
        ...
        <div class="main-header">
            <h1>🎬 Actor Face Swap Studio V3</h1>
            <p>Remplacez le visage d'un acteur dans vos vidéos avec intelligence artificielle</p>
            <p style="font-size: 0.9em; opacity: 0.9;">Propulsé par FaceFusion CLI | V3 avec Multi-Test</p>
        </div>
        """)
